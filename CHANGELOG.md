...
### Changed
- **W4 memory/GC hardening — shape slots are now the single authoritative property store.** Removed the parallel `props` hashmap that mirrored `slots` on every `JsObject` (the double-storage flagged in AGENTS.md). `getOwn`/`set`/`hasOwn`/`getLength`/`deleteOwn` now read/write only shape-indexed `slots`; ordered enumeration goes through `shape.key_order` via the new `JsObject.ownKeys()` helper. GC marking (`gc.zig traceJsObject`, `heap.zig markObject` both branches) is shape-aware: it walks `obj.slots` directly instead of iterating the old hashmap; sweep no longer `props.deinit`s. All property consumers migrated: `array_proto` (24 index lookups → `getOwn`, pop → `deleteOwn`), `object_methods` (keys/values/entries/fromEntries), `json` stringify, `promise`, `realm` prototype copy, and the bc for-in loop. 35 `.props` sites converted; the field is gone. **CI:** `gc-stress` promoted to a hard gate on all platforms + `differential` added (ubuntu). Verified: `zig build test` (leak-clean), `differential` 97/97, `gc-stress` bounded (<200 live). ASAN/UBSAN gate + footprint-vs-QuickJS measurement deferred.
- **W2 engine unification — the bytecode VM is now the default engine.** `Isolate.eval`, `Context.interp_mode`, and the CLI/REPL default switched from the tree-walker to bc (`--interp=tree` still selects the legacy tree-walker). To make this safe, the bc VM gained full **class/`new`/prototype** support (the last major parity gap):
  - bc functions are now first-class objects: `BcClosure` carries a lazily-created backing `JsObject` for own properties, so `C.prototype`, `C.prototype.m = …`, and static props (`C.k = …`) work (previously silent no-ops). `getProp`/`setProp` route through it; `prototype` is materialized on first access with a `constructor` back-reference.
  - `[[Construct]]` (`new C()`) is correct: the instance's prototype is the constructor's `.prototype`, `this` is bound, the constructor runs synchronously, and the result is the constructor's return value only if it is an object (else the new instance). Previously `new C()` returned the constructor's last-expression value (e.g. a number) with an `object_prototype` proto.
  - `instanceof` resolves a bc constructor's `.prototype` from its backing object. GC traces the backing object (and transitively its prototype + methods).
  - This makes `class`/`extends`/`super`/methods (which desugar to functions + prototype assignment) work end-to-end in bc; verified against Node.js (new `class_basic`/`class_extends_super`/`proto_constructor` differential corpus entries).
  - **Test oracle migrated to bc.** The integration suite's dual-mode helpers now run bc only (bc is the engine of record, validated against Node by the differential harness); the differential harness gates on bc-vs-Node and reports tree-vs-Node mismatches as informational. The tree-walker is being retired — it carries latent bugs the bc VM does not (e.g. `instanceof` across a default-constructor `extends` chain returned a wrong result; bc matches Node). It remains reachable via `--interp=tree` and a handful of explicit tree tests pending full removal.
- **Tree-walker removed (final W2 cleanup).** Deleted `src/vm/vm.zig` (the Phase 1 tree-walking evaluator) and the `.tree` `InterpMode` variant (`InterpMode = enum { bc }`). Dropped the `.tree` arm of `evalWithMode`, the `--interp=tree` CLI flag, the tree leg of the differential harness (now bc-vs-Node only), ~706 lines of explicit tree-mode integration tests + 3 GC tree tests, the tree-walker `Frame` struct in `frame.zig`, and the tree benchmark legs (`fib20`/`phase6_ic` are bc-only). `formatNumber` relocated from `vm.zig` to `value.zig` (used by `root.zig`/`realm.zig`). bc is now the only engine. `zig build test` + `differential` (97/97) green.

### Added
- **Real async/await in the bytecode VM (reaction-driven coroutines).** `async function` declarations/expressions and `async` arrow functions now parse (contextual `async` keyword with single-token lookahead + backtrack, so `async` is still usable as an identifier), carrying an `is_async` flag through the AST → `BcFunction`. Inside an async body the parser's `await x` desugar (`__await__(x)`) compiles to a `YIELD` suspend; calling an async function builds a coroutine (shared `buildGenState`) plus a pending result `Promise` and drives it: each `await` subscribes the awaited value's settlement (`Promise.resolve(v).then(onFulfilled, onRejected)`) to resume the coroutine via the microtask queue, so suspension/ordering match real async semantics. A rejected await throws **at the await point** (the new `runSuspendable` injects the exception into the suspended frame's try stack), so `try { await p } catch(e)` works. The driver settles the result promise on completion/throw, and `return await p` adopts a returned promise. New promise hooks (`newPendingPromise`/`settleResult`/`subscribeAwait`), VM `AsyncCtx`/`ResumeKind`/`SuspendResult`, and GC scanning of in-flight result promises. Top-level `await` keeps its synchronous-drain behaviour; the tree-walker still runs async bodies synchronously. 7 bc integration tests (basic/sequential/loop awaits, async arrow, rejected-await caught, nested async composition, thenable result). `zig build test` + `differential` (94/94) green.
- **Promise microtask fix (latent).** `runReactionJob` now snapshots the job's reaction fields into locals before invoking any callback. A reaction can enqueue more jobs and reallocate the `microtasks` buffer; because the job argument may be passed by reference to an element of that buffer, touching it afterward read freed memory. Harmless for shallow `.then` chains, but deep async/await chains hit it (segfault). Fixed at the source so all promise consumers benefit.

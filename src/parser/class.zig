...
/// Handles both named `class MyClass {}` and anonymous `class {}` expressions.
pub fn parseClassExpr(p: *Parser) ?*Node {
    const start = p.current.start;
    _ = p.advance(); // consume 'class'

    // Determine class name: use explicit if present, otherwise synthetic
    var class_name: []const u8 = undefined;
    var has_explicit_name = false;
    if (p.check(.identifier)) {
        class_name = p.current.value_str;
        has_explicit_name = true;
        _ = p.advance();
    } else {
        // Anonymous class: generate synthetic name __AnonClass<id>
        // Use string allocation from arena for stability
        class_name = p.arena.dupe(u8, "__AnonClassExpr") catch return null;
    }
    ...
    // Build default constructor if needed
    if (ctor_body.len == 0) {
        if (super_name) |sname| {
            const id_reflect = p.makeNode(.identifier, start, start, .{ .identifier = "Reflect" }) orelse return null;
            const id_construct = p.makeNode(.identifier, start, start, .{ .identifier = "construct" }) orelse return null;
    ...
    // Build constructor function body with super() binding
    var out = std.ArrayList(*Node){};
    var ctor_body_effective = ctor_body;
    if (super_name) |sname| {
        var ctor_stmts = std.ArrayList(*Node){};
    ...
    // Emit method assignments: ClassName.prototype.methodName = function(...) { ... };

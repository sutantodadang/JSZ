...
pub const JsObject = struct {
    ...
    internal_kind: enum(u8) { none, regexp, bound_function, date, map, set, weakmap, weakset, weakref, finalization_registry, promise, generator, async_generator, return_completion, proxy, array_buffer, typed_array, data_view, shared_array_buffer, module_namespace, shadow_realm, wrapped_function, mapped_arguments, map_iterator, set_iterator, array_iterator, iterator_helper, regexp_string_iterator, temporal_instant, temporal_duration, temporal_plain_date, temporal_plain_time, temporal_plain_date_time, temporal_zoned_date_time, temporal_plain_year_month, temporal_plain_month_day, promise_resolver, disposable_stack, async_disposable_stack, htmldda, proxy_revoke } = .none,
    ...
    /// `.bc_function` Value the program holds, and this object, which is what a
    /// `[[Prototype]]` slot can point at. Anything that turns a stored object
    /// pointer back into a Value must consult this so the function's identity
